...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        //Custom made
        private WeaponBuff m_CurrentBuff;
        private float m_BuffEndTime;

        public void ApplyWeaponBuff(WeaponBuff buff)
        {
            m_CurrentBuff = buff;
            m_BuffEndTime = Time.time + buff.Duration;

            // Apply to current weapon immediately
            WeaponController activeWeapon = GetActiveWeapon();
            if (activeWeapon != null)
            {
                activeWeapon.ApplyBuff(buff.DamageMultiplier, buff.FireRateMultiplier, buff.InfiniteAmmo, buff.Duration);
            }
        }

        private void ClearWeaponBuff()
        {
            m_CurrentBuff = null;

            // Clear buff from all weapons
        ...
        void Update()
        {
            ...
            if (m_CurrentBuff != null && Time.time > m_BuffEndTime)
            {
                ClearWeaponBuff();
            }
