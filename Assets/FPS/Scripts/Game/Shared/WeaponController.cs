...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        private float m_OriginalDelayBetweenShots;
        private float m_CurrentFireRateMultiplier = 1f;
        ...
        public bool IsBuffed => m_IsBuffed;
        public float CurrentFireRateMultiplier => m_CurrentFireRateMultiplier;
        public bool HasInfiniteAmmo => m_HasInfiniteAmmo;
        public float GetDamageMultiplier() => m_DamageMultiplier;

        public void ApplyBuff(float damageMultiplier, float fireRateMultiplier, bool infiniteAmmo, float duration)
        {
            // Store original values if this is the first buff
            if (!m_IsBuffed)
            {
                m_OriginalDelayBetweenShots = DelayBetweenShots;
            }

            m_DamageMultiplier = damageMultiplier;
            m_CurrentFireRateMultiplier = fireRateMultiplier;
            DelayBetweenShots = m_OriginalDelayBetweenShots * (1f / fireRateMultiplier); // Inverse because lower delay = faster rate
            m_HasInfiniteAmmo = infiniteAmmo;
        ...
        public void ClearBuff()
        {
            if (m_IsBuffed)
            {
                ...
                DelayBetweenShots = m_OriginalDelayBetweenShots;
        ...
        void Reload()
        {
            ...
            else if (m_CarriedPhysicalBullets > 0)
            {
        ...
        void Update()
        {
            ...
            if (Time.time > m_BuffEndTime && m_IsBuffed)
            {
                ClearBuff();
            }
        }
        ...
        bool TryShoot()
        {
            if ((m_CurrentAmmo >= 1f || m_HasInfiniteAmmo) // Modified condition
        && m_LastTimeShot + DelayBetweenShots < Time.time)
            {
                HandleShoot();
                if (!m_HasInfiniteAmmo) // Only consume ammo if not infinite
                {
                    m_CurrentAmmo -= 1f;
                }
                return true;
        ...
        {
            if (!IsCharging
        && (m_CurrentAmmo >= AmmoUsedOnStartCharge || m_HasInfiniteAmmo)
